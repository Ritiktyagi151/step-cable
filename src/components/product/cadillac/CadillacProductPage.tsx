...
function CadillacProductCard({ product }: { product: CadillacProduct }) {
  return (
    <Link
      ...
      className="group flex min-h-[360px] flex-col overflow-hidden rounded-[8px] border border-slate-200 bg-white shadow-lg shadow-slate-900/5 transition duration-300 hover:-translate-y-1 hover:border-[#2486FE]/35 hover:shadow-2xl hover:shadow-[#2486FE]/10"
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#2486FE]">{product.code || "STEP Cadillac"}</p>
      ...
      <section className="bg-[#f5f4f1] px-4 py-14 sm:px-6 sm:py-18 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-6 border-b border-slate-300/70 pb-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2486FE]">Product List</p>
            ...
            {productGroups.map((group) => (
              <section key={group.category} id={slugifyGroup(group.category)} className="scroll-mt-24">
                <div className="mb-5 flex flex-col gap-2 border-b border-slate-300/70 pb-4 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.24em] text-[#2486FE]">Cadillac Group</p>
            ...
            {pdfLinks.length ? (
              <div className="rounded-[8px] border border-slate-200 bg-[#f7f8fb] p-5 shadow-xl shadow-slate-900/5">
                ...
                <div className="mt-4 grid gap-3">
                  {pdfLinks.map((link) => (
                    <a key={link.href} href={link.href} target="_blank" className="inline-flex items-center justify-between gap-3 rounded-[8px] border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition hover:border-[#2486FE]/35 hover:text-[#2486FE]">
