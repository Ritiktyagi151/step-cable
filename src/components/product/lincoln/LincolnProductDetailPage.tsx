...
export function LincolnProductDetailPage({ page, product }: LincolnProductDetailPageProps) {
  const products = getLincolnProducts(page).filter((item) => item.slug !== product.slug);

  return (
    <main className="bg-[#f5f4f1]">
      <section className="px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <Link href="/step-lincoln" className="inline-flex items-center gap-2 text-sm font-black text-slate-700 transition hover:text-[#5BC0BB]">
          ...
          <div className="mt-8 grid overflow-hidden rounded-[8px] border border-slate-200 bg-white shadow-2xl shadow-slate-900/10 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="flex flex-col justify-center p-6 sm:p-8 lg:p-12">
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#5BC0BB]">STEP Lincoln Product</p>
      ...
      <MoreProductsSlider products={products} title="More Lincoln Products" basePath="/step-lincoln" fallbackCode="STEP Lincoln" />
    </main>
  );
}
